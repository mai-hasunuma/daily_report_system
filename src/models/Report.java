...
@NamedQueries({
    ...
    @NamedQuery(
            name = "getMyFollowingAllReports",
            query = "SELECT r FROM Report AS r WHERE r.employee.id = :followed_id ORDER BY r.id  DESC"
            ),
    @NamedQuery(
...
public class Report {
    ...
    @ManyToOne
    // 日報を作成するときはログインしているユーザ情報がそのまま格納される
    @JoinColumn(name = "employee_id", nullable = false)
    private Employee employee;

    // Dateは年月日のみを管理する　日報は何日の日報かわかればよいのでDateにする
    // Timestampはミリ秒まで情報を管理する
